  ...
  return (
    <div style={{ background: '#2C1A0E', borderRadius: 12, padding: '16px 18px', display: 'flex', gap: 16, alignItems: 'center' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, color: 'rgba(245,240,232,0.5)', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        <div style={{ fontSize: 12, color: 'rgba(245,240,232,0.6)' }}>{weather.conditionLabel}</div>
      </div>
      {weather.precipMm > 0 && (
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#E8A870' }}>{weather.precipMm}mm</div>
          <div style={{ fontSize: 10, color: 'rgba(245,240,232,0.4)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Rain</div>
        </div>
      )}
    </div>
  );
